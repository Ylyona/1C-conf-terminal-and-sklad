	...
	Функция ПолучениеОсновныхДеталей(Знач Статус, Знач НазваниеРесурса = Null, Знач ЗначениеРесурса = Null) Экспорт
		...
		ИначеЕсли НазваниеРесурса = Null и ЗначениеРесурса = Null Тогда
			Запрос = Новый Запрос(
			"ВЫБРАТЬ
			|ДвижениеОсновныхДеталейСрезПоследних.Деталь КАК Деталь 
			|ИЗ 
	...
	// Функция - Получение пустой ссылки справочника основные детали
	// 
	// Возвращаемое значение:
	// 	Ссылка - Пустая ссылка справочника основные средства
	//
	&НаСервере
	Функция ПолучениеПустойСсылкиСправочникаОсновныеСредства() Экспорт
		Возврат Справочники.ОсновныеСредства.ПустаяСсылка();
	КонецФункции
	
	// Функция - Получение неисправности основной детали
	//
	// Параметры:
	//  Дата	 - Дата	 - Время на которое комментарий должен быть актуален
	//  Деталь	 - Ссылка	 - Деталь сведенья о неисправности которой необходимо получить
	// 
	// Возвращаемое значение:
	//  ЭлементСпискаЗначений - Полученное поясние неисправности детали
	//
	&НаСервере
	Функция ПолучениеНеисправностиОсновнойДетали(Знач Дата, Знач Деталь) Экспорт
		ПараметрыОтбораИзРегистраСведений = Новый Структура;
		ПараметрыОтбораИзРегистраСведений.Вставить("Деталь", Деталь);
		АктуальныеДанныеОбОсновнойДетали = РегистрыСведений.ДвижениеОсновныхДеталей.СрезПоследних(Дата, 
		ПараметрыОтбораИзРегистраСведений);
		АктуальныйКомментарийИзРегистра = АктуальныеДанныеОбОсновнойДетали.ВыгрузитьКолонку("Комментарий");
		ДоступныеЗначенияКомментария = новый СписокЗначений;
		ДоступныеЗначенияКомментария.ЗагрузитьЗначения(АктуальныйКомментарийИзРегистра);
		Возврат ДоступныеЗначенияКомментария;
	КонецФункции