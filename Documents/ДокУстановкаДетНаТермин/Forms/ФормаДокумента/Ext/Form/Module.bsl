#Область ПроцедурыФормы
	...
	Процедура ПодготовкаФормы()
		...
		Если Объект.ИсточникПоступленияВТермин = ПеречислВЗ тогда
		
			Объект.Терминал = ПолучитьПустуюСсылкуТерминала();
			Элементы.Терминал.Видимость = Ложь;
			
		ИначеЕсли Объект.ИсточникПоступленияВТермин = ПеречислИсп тогда
			
			Элементы.Терминал.Видимость = Истина;
		
		ИначеЕсли Объект.ИсточникПоступленияВТермин = ПеречислСкл Или Объект.ИсточникПоступленияВТермин.Пустая() тогда
	...
	Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
		Если Элементы.Терминал.Видимость = Истина и Объект.Терминал.Пустая() тогда
			Отказ = истина;
			Сообщить("Укажите Терминал!");
		Иначе 
			Отказ = ложь;
		КонецЕсли;
...
#Область ПроцедурыТабличнойЧастиРасходныеДетали
	...
	&НаКлиенте
	Процедура РасходныеДетали1ДетальРасхВТерминНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
		СтандартнаяОбработка = ложь;
		Источник = Объект.ИсточникПоступленияВТермин;
	...
	&НаКлиенте
	Процедура РасходныеДетали1ДетальРасхВТерминПриИзменении(Элемент)
		...
		Если Источник = ПолучитьПеречислениеНаСклад() или не Объект.Менеджер.Пустая() или не Объект.Терминал.Пустая() тогда
			...
			КонецЕсли;
			ТД.КоличествоДоступноДляТермин = ПолучитьКоличествоДеталей(Объект.Дата,Источник,ТД.ДетальРасхВТермин,ЗначИзмер);
		Иначе
			Сообщить("Не верно заполнены данные!!!");
		КонецЕсли;
	КонецПроцедуры
	...
	&НаКлиенте
	Процедура РасходныеДетали1КоличествоРасхВТерминПриИзменении(Элемент)
		ТД1 = Элементы.РасходныеДетали1.ТекущиеДанные;
		ОбщееКоличествоОдинаковыхРасходныхДеталей = ТД1.КоличествоРасхВТермин;
		Ошибка = Ложь;
		Если ТД1.КоличествоДоступноДляТермин < ТД1.КоличествоРасхВТермин тогда
			ТД1.КоличествоРасхВТермин = 0;
			Сообщить("Данного количества детали "+ТД1.ДетальРасхВТермин+" нет в наличии");
			Ошибка = Истина;
		КонецЕсли;
		Если Ошибка = Ложь Тогда
			Для Индекс = 0 По Объект.РасходныеДетали.Количество()-2 Цикл
				//Сравниваем наименование деталей, если совпадает то проверяем
				Если ТД1.ДетальРасхВТермин = Объект.РасходныеДетали.Получить(Индекс).ДетальРасхВТермин Тогда
					//Складываем то количество детали которое запомнили (вводили ранее) с тем что ввели сейчас
					ОбщееКоличествоОдинаковыхРасходныхДеталей = ОбщееКоличествоОдинаковыхРасходныхДеталей + Объект.РасходныеДетали.Получить(Индекс).КоличествоРасхВТермин;
					//Если Доступное количество меньше того что получается при сложении то возвращаем нуль
					Если ТД1.КоличествоДоступноДляТермин < ОбщееКоличествоОдинаковыхРасходныхДеталей Тогда
						ТД1.КоличествоРасхВТермин = 0;
						Сообщить("Данного количества детали "+ТД1.ДетальРасхВТермин+" нет в наличии, пороверьте предыдущие строки");
						Прервать;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
...
#Область ПроцедурыТабличнойЧастиОсновныеДетали
	...
	Процедура ОсновныеДетали1ДетальПриемНаСклПриИзменении(Элемент)
		...
		Для Индекс = 0 По Объект.ОсновныеДетали.Количество()-2 Цикл
			Если ТД.ДетальУстанВТермин = Объект.ОсновныеДетали.Получить(Индекс).ДетальУстанВТермин Тогда
				ТД.ДетальУстанВТермин = получитьПустуюСсылкуОснДет();
