#Область ПроцедурыФормы
	...
	Процедура ПодготовкаФормы()
		...
		Если Объект.ИсточникПоступленияНеисправно = ПеречислВЗ тогда
		...
		ИначеЕсли Объект.ИсточникПоступленияНеисправно = ПеречислИсп тогда
			
			Элементы.Терминал.Видимость = Истина;
			
			Элементы.Менеджер.Видимость = Истина;
			
		ИначеЕсли Объект.ИсточникПоступленияНеисправно = ПеречислСкл Или Объект.ИсточникПоступленияНеисправно.Пустая() тогда
...
#Область ПроцедурыТабличнойЧастиРасходныеДетали
	...
	&НаКлиенте
	Процедура РасходныеДетали1ДетальРасходНеиспрНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
		СтандартнаяОбработка = ложь;
		Источник = Объект.ИсточникПоступленияНеисправно;
		Если Источник = ПолучитьПеречислениеНаСклад()  или не Объект.Менеджер.Пустая() или не Объект.Терминал.Пустая() тогда
			Если  Источник = ПолучитьПеречислениеВЗапас() тогда
			...
			ИначеЕсли Источник = ПолучитьПеречислениеНаСклад() тогда
	...
	&НаКлиенте
	Процедура РасходныеДетали1ДетальРасходНеиспрПриИзменении(Элемент)
		...
		Если Источник = ПолучитьПеречислениеНаСклад() или не Объект.Менеджер.Пустая() или не Объект.Терминал.Пустая() тогда
			...
			КонецЕсли;
			ТД.КоличествоРасхНеиспрДоступно = ПолучитьКоличествоДеталей(Объект.Дата,Источник,ТД.ДетальРасходНеиспр,ЗначИзмер);
		Иначе
			Сообщить("Не верно заполнены данные!!!");
		КонецЕсли;
	КонецПроцедуры
	...
	&НаКлиенте
	Процедура РасходныеДетали1КоличествоРасхНеиспрПриИзменении(Элемент)
		ТД1 = Элементы.РасходныеДетали1.ТекущиеДанные;
		ОбщееКоличествоОдинаковыхРасходныхДеталей = ТД1.КоличествоРасхНеиспр;
		Ошибка = Ложь;
		Если ТД1.КоличествоРасхНеиспрДоступно < ТД1.КоличествоРасхНеиспр тогда
			ТД1.КоличествоРасхНеиспр = 0;
			Сообщить("Данного количества детали "+ТД1.ДетальРасходНеиспр+" нет в наличии");
			Ошибка = Истина;
		КонецЕсли;
		Если Ошибка = Ложь Тогда
			Для Индекс = 0 По Объект.РасходныеДетали.Количество()-2 Цикл
				Если ТД1.ДетальРасходНеиспр = Объект.РасходныеДетали.Получить(Индекс).ДетальРасходНеиспр Тогда
					ОбщееКоличествоОдинаковыхРасходныхДеталей = ОбщееКоличествоОдинаковыхРасходныхДеталей + Объект.РасходныеДетали.Получить(Индекс).КоличествоРасхНеиспр;
					Если ТД1.КоличествоРасхНеиспрДоступно < ОбщееКоличествоОдинаковыхРасходныхДеталей Тогда
						ТД1.КоличествоРасхНеиспр = 0;
						Сообщить("Данного количества детали "+ТД1.ДетальРасходНеиспр+" нет в наличии, пороверьте предыдущие строки");
						Прервать;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
...
#Область ПроцедурыТабличнойЧастиОсновныеДетали
	...
	Процедура ОсновныеДетали1ДетальПриемНаСклПриИзменении(Элемент)
		...
		Для Индекс = 0 По Объект.ОсновныеДетали.Количество()-2 Цикл
			Если ТД.ДетальПередатьНеиспр = Объект.ОсновныеДетали.Получить(Индекс).ДетальПередатьНеиспр Тогда
				ТД.ДетальПередатьНеиспр = получитьПустуюСсылкуОснДет();
